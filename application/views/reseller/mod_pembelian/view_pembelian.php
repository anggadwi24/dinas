              ...
              <div class="box">
                ...
                <div class="box-body">
                  ...
                  <?php 
                    ...
                    elseif($row['proses']=='1'){ $proses = '<i class="text-success">Proses</i>'; }
                    elseif($row['proses']=='2'){ $proses = '<i class="text-info">Konfirmasi</i>'; }
                    elseif($row['proses']=='3'){$proses = 'Sudah Dibayar';}
                    elseif($row['proses']=='4'){$proses = $row['no_resi'].'<i class="text-success">Dalam Perjalanan</i>';}
                    else{ $proses = '<i class="text-success">Pesanan Selesai</i>';}

                    $total = $this->db->query("SELECT sum((a.harga_jual*a.jumlah)-a.diskon) as total FROM `rb_penjualan_detail` a where a.id_penjualan='$row[id_penjualan]'")->row_array();
                    if ($row['service']==''){ $service = "<i style='color:green'>Pembelian ke Pusat</i>"; }
                    echo "<tr><td>$no</td>
                              <td>$row[kode_transaksi]</td>
                              <td>$row[waktu_transaksi]</td>
                              <td>$proses</td>
                              <td style='color:red;'>Rp ".rupiah($total['total']+$row['ongkir'])."</td>
                              <td>".strtoupper($row['kurir'].'-'.$row['service']) ."</td>

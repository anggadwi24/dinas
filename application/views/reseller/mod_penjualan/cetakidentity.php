 ...
 <?php $res = $this->db->query("SELECT a.*, b.nama_kota, c.nama_provinsi FROM rb_reseller a JOIN rb_kota b ON a.kota_id=b.kota_id 
  ...
  <?php $usr = $this->db->query("SELECT a.*, b.nama_kota, c.nama_provinsi FROM rb_konsumen a JOIN rb_kota b ON a.kota_id=b.kota_id 
                JOIN rb_provinsi c ON b.provinsi_id=c.provinsi_id
                  where a.id_konsumen='".$rows['id_pembeli']."'")->row_array(); ?>
  <?php $dropship = $this->db->query("SELECT * FROM rb_dropshipper_history WHERE kode_transaksi = '".$detail['kode_transaksi']."'");?>
  <?php $disk = $this->db->query("SELECT * FROM rb_produk_diskon where id_produk='$row[id_produk]'")->row_array();
   ...
   <?php if($dropship->num_rows() > 0){

              $drp = $dropship->row_array();
                 $kota = $this->db->query("SELECT a.*, b.nama_kota, c.nama_provinsi FROM rb_dropshipper_history a JOIN rb_kota b ON a.kota_id=b.kota_id 
                        JOIN rb_provinsi c ON b.provinsi_id=c.provinsi_id
                          where a.id_dh='".$drp['id_dh']."'")->row_array();


                $nama = $drp['nama_penerima'];
                $hp = $drp['no_hp'];
                $alamat = $drp['alamat'];
                $kota = $kota['nama_kota'];
                $provinsi = $kota['nama_provinsi'];
                $kecamatan = $drp['kecamatan'];

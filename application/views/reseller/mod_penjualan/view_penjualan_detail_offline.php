              ...
              <div class="box">
                ...
                <div class="box-body">
                  ...
                  <tbody>
                    ...
                    <tr><th scope='row'>Status</th>                        <td>
                      ...
                      <?php
                        ...
                        elseif($rows['proses']=='3'){$status = 'Pembayaran Masuk'; $proses = '<i class="text-success">Pembayaran Masuk</i>';$ubah = 1;}
                    ...
                    <tr><th scope="row">Action</th><td><?php if($rows['proses']==2){?><a href="<?= base_url('reseller/proses_penjualan_detail').'/'.$rows[id_penjualan].'/3' ?>" class="btn btn-primary btn-xs">Sudah Dibayar</a><?php }elseif($rows['proses']==3){?> 
                        ...
                        Masukan No Resi : <input type="text" name="noresi" class="form-control"> <input type="hidden" name="id_penjualan" value="<?= $rows[id_penjualan]?>"> <input type="hidden" name="status" value="offline">

                        </form>
                      <?php }elseif($rows['proses']=='4'){ ?><i class="text-success"><?= $rows['no_resi']?></i><br><a href="<?= base_url('reseller/proses_penjualan_detail').'/'.$rows[id_penjualan].'/5/'.$rows[kode_transaksi].'/offline' ?>" class="btn btn-success btn-xs">SAMPAI</a><?php }else{ echo $rows['no_resi'];}?></td></tr>
                  </tbody>
                  </table>
                  <hr>
                  ...
                  <table class="table table-condensed table-bordered">
                    <tbody>
                      ...
                      <?php $res = $this->db->query("SELECT a.*, b.nama_kota, c.nama_provinsi FROM rb_konsumen_offline a JOIN rb_kota b ON a.kota_id=b.kota_id 
                JOIN rb_provinsi c ON b.provinsi_id=c.provinsi_id
                  where a.rbo_kode_transaksi='$rows[kode_transaksi]'")->row_array(); ?>
                      ...
                      <tr><th style="width: 150px;">Alamat</th><td><?= $rows['alamat'].' / '.$res['nama_provinsi'].' / '.$res['nama_kota'].' / '.$rows['kecamatan']?></td></tr>
                ...
                <br>
                <a href="<?php echo base_url('reseller/cetakdataoff').'/'.$rows[id_penjualan].'/'.$rows['kode_transaksi']?>" class="pull-right"><span class="glyphicon glyphicon-print"></span></a>
              </div>

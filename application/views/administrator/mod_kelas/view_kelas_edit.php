                        ...
                        <div class="card-body">
                               ...
                               <div class="row">
                                ...
                                <div class="col-md-12 form-group">
                                    ...
                                    <?php }else{echo "<input type='hidden' name='id_sd' value='".$row['rk_id_sd']."'>";}?>
                                   ...
                                   <div class="col-md-12 form-group">
                                       <label for="">Judul Kelas</label>
                                        <input type="text" name="judul" class="form-control" placeholder="Judul Kelas" value="<?= $row['rk_title']?>">
                                        
                                   </div>
                                   <div class="col-md-12 form-group">
                                       <label for="">Deskripsi Kelas</label>
                                        <textarea name="deskripsi" class="form-control" placeholder="Deskripsi Kelas"><?= $row['rk_desc']?></textarea>
                                   </div>
                                   <div class="col-md-12 form-group">
                                   ...
                                   <div class="col-md-8 form-group">
                                       ...
                                       <input type="text" class="form-control" id="kode" name="code" placeholder="Kode Kelas" required value="<?= $row['rk_code']?>"> 
                                   </div>
                                   <div class="col-md-4 form-group">
                                       <input type="hidden" name="id" value="<?= encode($row['rk_id'])?>">
...
<script>
    var cabang = $('#cabang').val(<?= $row['rk_id_sd']?>);
    $('#accept').val('<?= $row['rk_acc']?>').change();;

    getCabang(<?= $row['rk_id_sd'] ?>);
    function getCabang(id){
        $.ajax({
            ...
            success:function(resp){
                ...
                $('#kelas').val('<?= $row['rk_kelas']?>');
               mapel('<?= $row['rk_kelas']?>','<?= $row['rk_mapel']?>');

            }
        })
    }
    getGuru('<?= $row['rk_id_sd'] ?>','<?= $row['rk_id_guru'] ?>')
    function getGuru(id,guru){
        $.ajax({
            type:'POST',
            url:'<?= base_url('administrator/getGuruNotNull') ?>',
            data:{id:id,guru:guru},
            dataType:'json',
            success:function(resp){
                $('#guru').html(resp);
            }
    ...
    function mapel(kelas,mapel){
        $.ajax({
            type:'POST',
            url:'<?= base_url('administrator/getMapelNotNull') ?>',
    ...
    $("#formAdd").on('submit', function(e){
        ...
        $.ajax({
            ...
            type: 'POST',
            url:"<?= base_url('administrator/updateClass') ?>",

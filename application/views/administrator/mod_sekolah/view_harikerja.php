                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <form method="post" action="<?= base_url('administrator/workingDays')?>">
                          <div class="row">
                                ...
                                <?php 
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="form-check">
                                          <input class="form-check-input" id="kerja<?=$row[id_wd]?>" type="radio" name="status<?=$row[id_wd]?>" onClick="processForm1(<?=$row[id_wd]?>)" value="kerja" <?php if($row['status']=='kerja'){echo"checked";}?>>
                                          <label class="form-check-label" for="kerja<?=$row[id_wd]?>">
                                           Kerja
                                          </label>
                                        
                                        </div>
                                        <div class="form-check">
                                          <input class="form-check-input" id="libur<?=$row[id_wd]?>" type="radio" name="status<?=$row[id_wd]?>" onClick="processForm(<?=$row[id_wd]?>)" value="libur" data-id='<?= $row[id_wd]?>'  <?php if($row['status']=='libur'){echo"checked";}?> >
                                          <label class="form-check-label" for="libur<?=$row[id_wd]?>">
                                           Libur
                                          </label>
                                        </div>
                                    </div>
...
<script type="text/javascript">
   function processForm1(id) { 
    ...
    console.log(status);
     $.ajax( {
        type: 'POST',
        url: '<?= base_url('administrator/updateWorkingDays')?>',

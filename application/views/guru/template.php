 ...
 <section id="hero" class="d-flex align-items-center">
    <div class="container position-relative" data-aos="fade-up" data-aos-delay="100">
      <div class="row justify-content-center">
        ...
        <div class="col-4">
          <img src="<?= $foto?>" class="rounded-circle mt-4" style="width: 80px;" alt="Cinque Terre">
        </div>
      </div>
...
<?php $h = date('w');
     ...
     $harikerja = $this->model_app->view_where('working_hours',array('hari'=>$h));
     $hk = $harikerja->row_array();
     $bulan = date('m');
     $tgl = date('j');
     $id = $this->session->userdata['guru']['id_guru'];
     $hadir = $this->db->query("SELECT * FROM absensi WHERE id_pegawai = $id AND MONTH(tanggal) = $bulan AND status_absen ='guru' ")->num_rows();
     $form_i = $this->db->query("SELECT * FROM form_izin WHERE id_pegawai = $id AND MONTH(dari) = $bulan AND MONTH(sampai) = $bulan AND status='Izin' AND status_form = 'guru' ");
      $form_s = $this->db->query("SELECT * FROM form_izin WHERE id_pegawai = $id AND MONTH(dari) = $bulan AND MONTH(sampai) = $bulan AND status='Sakit' AND status_form = 'guru' ");
      $ijin = 0;
      ...
      $harikerja1 = $this->db->query("SELECT * FROM working_days WHERE bulan = $bulan AND tanggal <= $tgl AND tahun = '".date('Y')."' AND status = 'kerja' ");
      $alpha = 0;
      foreach($harikerja1->result_array() as $hi){
        $tgl = $hi['tahun'].'-'.$hi['bulan'].'-'.$hi['tanggal'];
        $date = date('Y-m-d',strtotime($tgl));
        $abs = $this->db->query("SELECT * FROM absensi WHERE id_pegawai = $id AND tanggal = '$date' AND status_absen = 'guru' ");
        if($abs->num_rows() <= 0 )
        {
          $alpha += 1;
        }else{
 ...
 <section id="about" class="about position-relative" style="margin-top: -30px !important;padding:0px !important;">
      ...
      <?php $lembur = $this->model_app->view_where('lembur',array('id_pegawai'=>$this->session->userdata['guru']['id_guru'],'date'=>date('Y-m-d')));
        ...
        $form = $this->db->query("SELECT * FROM form_izin WHERE id_pegawai = $id_peg  AND sampai >= '$tgl' AND status_form = 'guru' ORDER BY id_form DESC LIMIT 3");
        if($form->num_rows() > 0){
        ?>
        <div class="row mt-3 justify-content-center">
      ...
      <div class="col-12 mt-5">
          <a href="<?= base_url('guru/logout')?>" class="btn btn-danger col-12">LOGOUT</a>
      </div>
      </div>
...
<?php $this->model_utama->kunjungan(); ?>
  ...
  <!-- <div id="fixedContainer">
    <a href="<?= base_url('guru/chat')?>"><span class="bg-warning p-2 rounded"><i class="ri-message-3-line text-light" style="font-size: 20px"></i></span></a>
  </div> -->
  ...
  <script>
   ...
   $.ajax({
       ...
       type:'POST',
       url:'<?= base_url('guru/dataCalendar') ?>',

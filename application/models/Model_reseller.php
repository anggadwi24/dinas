...
class Model_reseller extends CI_model{
    ...
    function penjualan_list_konsumen($id,$level){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen where a.status_penjual='$level' AND a.id_penjual='$id' AND a.proses > '1' ORDER BY a.id_penjualan DESC");
    }
    function penjualan_list_konsumen_off($id,$level){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen_offline b ON a.kode_transaksi=b.rbo_kode_transaksi where a.status_penjual='$level' AND a.id_penjual='$id' AND a.proses > '0' ORDER BY a.id_penjualan DESC");
    }
    function download_penjualan_list_konsumen($id,$level,$where,$proses){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen b ON a.id_pembeli=b.id_konsumen where a.status_penjual='$level' AND a.id_penjual='$id' $where  $proses ORDER BY a.id_penjualan DESC");
    }
    function download_penjualan_list_konsumen_off($id,$level,$where,$proses){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen_offline b ON a.kode_transaksi=b.rbo_kode_transaksi where a.status_penjual='$level' AND a.id_penjual='$id' $where  $proses ORDER BY a.id_penjualan DESC");
    }
    function jual($id){
    ...
    function jual_reseller($penjual, $produk){
        return $this->db->query("SELECT sum(jumlah) as jual FROM `rb_penjualan` a JOIN rb_penjualan_detail b ON a.id_penjualan=b.id_penjualan where a.status_pembeli='konsumen' AND a.status_penjual='reseller' AND a.id_penjual='$penjual' AND b.id_produk='$produk' AND a.proses>'2'");
    }
    ...
    function jual_admin($penjual, $produk){
        return $this->db->query("SELECT sum(jumlah) as jual FROM `rb_penjualan` a JOIN rb_penjualan_detail b ON a.id_penjualan=b.id_penjualan where a.status_pembeli='reseller' AND a.status_penjual='admin' AND a.id_penjual='$penjual' AND b.id_produk='$produk' AND a.proses>'2'");
    }

    function beli_admin($pembeli, $produk){
        return $this->db->query("SELECT sum(jumlah) as beli FROM `rb_penjualan` a JOIN rb_penjualan_detail b ON a.id_penjualan=b.id_penjualan where a.status_pembeli='reseller' AND a.status_penjual='admin' AND a.id_pembeli='$pembeli' AND b.id_produk='$produk' AND a.proses='1'");
    }
    ...
    function orders_report_dropship($id){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_dropshipper_history b ON a.kode_transaksi=b.kode_transaksi where  b.id_dropshipper='$id' AND a.status_pembeli='dropshipper' ORDER BY a.id_penjualan DESC");
    }
    ...
    function reseller_pembelian($id,$level){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_reseller b ON a.id_pembeli=b.id_reseller JOIN rb_penjualan_detail c ON a.id_penjualan = c.id_penjualan where a.status_penjual='$level' AND a.id_pembeli='$id' AND  c.harga_jual != 0 ORDER BY a.id_penjualan DESC");
    }
    ...
    function penjualan_konsumen_detail_reseller($id){
     ...
     function penjualan_konsumen_detail_reseller_off($id){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen_offline b ON a.kode_transaksi=b.rbo_kode_transaksi where a.id_penjualan='$id'");
    }
    function penjualan_konsumen_offline_detail_reseller($id){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_konsumen_offline b ON a.kode_transaksi=b.rbo_kode_transaksi where a.id_penjualan='$id'");
    }

    function penjualan_list($id,$level){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_reseller b ON a.id_pembeli=b.id_reseller where a.status_penjual='$level' AND a.id_penjual='$id' AND a.ongkir !=0  ORDER BY a.id_penjualan DESC");
    }
     function download_penjualan_list($id,$level,$where,$proses){
        return $this->db->query("SELECT * FROM `rb_penjualan` a JOIN rb_reseller b ON a.id_pembeli=b.id_reseller where a.status_penjual='$level' AND a.id_penjual='$id' $where $proses ORDER BY a.id_penjualan DESC");
    }

    function pembelian($id_reseller){
        return $this->db->query("SELECT sum((b.jumlah*b.harga_jual)-b.diskon+a.ongkir) as total FROM rb_penjualan a JOIN rb_penjualan_detail b ON a.id_penjualan=b.id_penjualan where a.status_penjual='admin' AND a.id_pembeli='".$id_reseller."' AND a.proses>'2'");
    }

    function penjualan_perusahaan($id_reseller){
        return $this->db->query("SELECT sum((a.jumlah*a.harga_jual)-a.diskon) as total, sum(a.jumlah) as produk FROM `rb_penjualan_detail` a JOIN rb_produk b ON a.id_produk=b.id_produk JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where c.status_penjual='reseller' AND b.id_produk_perusahaan='0' AND id_penjual='".$id_reseller."' AND c.proses>'2'");
    }
    ...
    function modal_perusahaan($id_reseller){
        return $this->db->query("SELECT sum(a.jumlah*b.harga_beli) as total FROM `rb_penjualan_detail` a JOIN rb_produk b ON a.id_produk=b.id_produk JOIN rb_penjualan c ON a.id_penjualan=c.id_penjualan where  c.proses > '2' AND c.id_penjual='".$id_reseller."' AND b.id_produk_perusahaan='0'");
    }
    ...
    function absensi_month($id,$month){
        return $this->db->query("SELECT * FROM absensi WHERE id_konsumen = $id AND MONTH(date) = $month ORDER BY date DESC");
    }

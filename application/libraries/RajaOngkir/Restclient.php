...
class RESTClient {
    ...
    public function __construct($api_key, $endpoint) {
        ...
        $this->api_url = "https://pro.rajaongkir.com/";
    }
    ...
    function post($params) {
        ...
        curl_setopt($curl, CURLOPT_URL, $this->api_url . 'api' . "/" . $this->endpoint);
    ...
    function get($params) {
        ...
        $query = http_build_query($params);
        curl_setopt($curl, CURLOPT_HTTPHEADER, $header);
        curl_setopt($curl, CURLOPT_URL, $this->api_url . 'api' . "/" . $this->endpoint . "?" . $query);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, TRUE);
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, FALSE);
        $request = curl_exec($curl);
        $return = ($request === FALSE) ? curl_error($curl) : $request;
